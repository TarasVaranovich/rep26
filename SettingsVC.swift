...
class SettingsVC: UIViewController ,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            let cell = Bundle.main.loadNibNamed("TrackingSwitchCell",
                                                owner: self, options: nil)?.first as! TrackingSwitchCell
            
            return cell
            
        } else {
        
            let cell = Bundle.main.loadNibNamed("ControlCell",
                                                owner: self, options: nil)?.first as!ControlCell
