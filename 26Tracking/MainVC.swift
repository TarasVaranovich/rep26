...
class MainVC: UIViewController, CLLocationManagerDelegate {
    ...
    var stringTimesSet = [String](repeating: " ", count: 1440)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if SettingsData.sharedInstance.setTracking{
            ...
            trackingTimer = Timer.scheduledTimer(timeInterval: TimeInterval(interval),
                                            target:self,
                                            selector: #selector(MainVC.saveCoordinates),
                                            userInfo: nil,
                                            repeats: true)
    ...
    func dateCasting(acceptedDate: NSDate) -> (NSDate?,NSDate?) {
        ...
        castingDateComponents.day = (castingCalendar?.component(.day,
                                                                from: castingDate as Date))!
        castingDateComponents.month = (castingCalendar?.component(.month,
                                                                  from: castingDate as Date))!
        castingDateComponents.year = (castingCalendar?.component(.year,
                                                                 from: castingDate as Date))!
